...
const[clFromLang,setClFromLang]=useState("python");
const[clToLang,setClToLang]=useState("javascript");
const[clResult,setClResult]=useState(null);
const[clLoading,setClLoading]=useState(false);
const[ghIssuesRepoUrl,setGhIssuesRepoUrl]=useState("");
const[ghIssuesResult,setGhIssuesResult]=useState(null);
const[ghIssuesLoading,setGhIssuesLoading]=useState(false);
const[selectedIssue,setSelectedIssue]=useState(null);
const[issueFixResult,setIssueFixResult]=useState(null);
...
const runSmartScan=async()=>{
  ...
  setRunHistory(h=>[{filename:files[0].name,timestamp:new Date().toLocaleTimeString(),issues:(results.security?.findings||[]).length,confidence:results.migrate?.confidence_score||95},...h].slice(0,5));
...
else if(mode==="regframework"){endpoint="/regulatory-framework";}
else if(mode==="codeqa"){endpoint="/ask-code-question";}
else if(mode==="sandbox"){endpoint="/sandbox-test";}
else if(mode==="quality"){endpoint="/code-quality";}
else if(mode==="smells"){endpoint="/code-smells";}
else if(mode==="refactor"){endpoint="/refactor-suggest";}
else if(mode==="platform"){endpoint="/platform-compatibility";}
else if(mode==="portability"){endpoint="/dependency-portability";}
else if(mode==="config"){endpoint="/config-migration";}
else if(mode==="readiness"){endpoint="/rearchitecture-readiness";}
...
else if(mode==="strangler"){endpoint="/strangler-fig";}
else if(mode==="debtcost"){endpoint="/tech-debt-cost";}
else if(mode==="dna"){endpoint="/code-dna";}
else if(mode==="depgraph"){endpoint="/dependency-graph";}
...
const fetchHistory=async()=>{
  ...
  try{
    const res=await fetch(API+"/codebase-history",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({repo_url:historyRepoUrl})});
    const data=await res.json();
    setHistoryResult(data);
  }catch(e){setHistoryResult({error:"Could not fetch history"});}
  setHistoryLoading(false);
};
const fetchGhIssues=async()=>{
  if(!ghIssuesRepoUrl||!ghIssuesRepoUrl.trim()){setGhIssuesResult({error:"Please enter a GitHub repo URL."});return;}
  setGhIssuesLoading(true);
  try{
    const res=await fetch(API+"/github-issues",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({repo_url:ghIssuesRepoUrl})});
    const data=await res.json();
    setGhIssuesResult(data);
  }catch(e){setGhIssuesResult({error:"Could not fetch issues"});}
  setGhIssuesLoading(false);
};
const fetchIssueFix=async(issue)=>{
  setSelectedIssue(issue);
  setIssueFixLoading(true);
  try{
    const res=await fetch(API+"/github-issue-fix",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({issue_title:issue.title,issue_body:issue.body})});
    const data=await res.json();
    setIssueFixResult(data);
  }catch(e){setIssueFixResult({error:"Could not get fix suggestion"});}
...
const fetchCrossLang=async()=>{
  ...
  try{
    ...
    const data=await res.json();
    setClResult(data);
  }catch(e){setClResult({error:"Could not translate code"});}
  setClLoading(false);
};
...
<div style={{marginBottom:"16px"}}><button onClick={()=>setCurrentPage("modernize")} style={{display:"block",width:"100%",textAlign:"left",padding:"7px 10px",borderRadius:"6px",border:"none",background:currentPage==="modernize"?"rgba(59,130,246,0.15)":"transparent",color:currentPage==="modernize"?"#60a5fa":"#94a3b8",cursor:"pointer",fontSize:"12px",fontWeight:"600",marginBottom:"3px"}}>Modernize Code</button><button onClick={fetchDashboard} style={{display:"block",width:"100%",textAlign:"left",padding:"7px 10px",borderRadius:"6px",border:"none",background:"transparent",color:"#94a3b8",cursor:"pointer",fontSize:"12px",fontWeight:"600",marginBottom:"3px"}}>Dashboard</button><button onClick={()=>{setCurrentPage("ask");setMode("codeqa");}} style={{display:"block",width:"100%",textAlign:"left",padding:"7px 10px",borderRadius:"6px",border:"none",background:currentPage==="ask"?"rgba(59,130,246,0.15)":"transparent",color:currentPage==="ask"?"#60a5fa":"#94a3b8",cursor:"pointer",fontSize:"12px",fontWeight:"600",marginBottom:"3px"}}>Ask Codebase</button><button onClick={()=>{setCurrentPage("docs");setMode("docs");}} style={{display:"block",width:"100%",textAlign:"left",padding:"7px 10px",borderRadius:"6px",border:"none",background:currentPage==="docs"?"rgba(59,130,246,0.15)":"transparent",color:currentPage==="docs"?"#60a5fa":"#94a3b8",cursor:"pointer",fontSize:"12px",fontWeight:"600",marginBottom:"3px"}}>Documentation</button><button onClick={()=>{setCurrentPage("roi");setMode("debtcost");}} style={{display:"block",width:"100%",textAlign:"left",padding:"7px 10px",borderRadius:"6px",border:"none",background:currentPage==="roi"?"rgba(59,130,246,0.15)":"transparent",color:currentPage==="roi"?"#60a5fa":"#94a3b8",cursor:"pointer",fontSize:"12px",fontWeight:"600"}}>ROI & Debt</button></div>
...
{mode==="timediff"&&<div style={{marginTop:"10px"}}><input type="text" value={diffRepoUrl} onChange={e=>setDiffRepoUrl(e.target.value)} placeholder="https://github.com/owner/repo" style={{width:"100%",padding:"8px 12px",borderRadius:"8px",border:"1px solid #be185d",background:codebg,color:text,fontSize:"13px",marginBottom:"6px"}}/><input type="text" value={diffFilePath} onChange={e=>setDiffFilePath(e.target.value)} placeholder="path/to/file.py" style={{width:"100%",padding:"8px 12px",borderRadius:"8px",border:"1px solid #be185d",background:codebg,color:text,fontSize:"13px",marginBottom:"6px"}}/><input type="text" value={diffCommitOld} onChange={e=>setDiffCommitOld(e.target.value)} placeholder="Old commit SHA" style={{width:"48%",padding:"8px 12px",borderRadius:"8px",border:"1px solid #be185d",background:codebg,color:text,fontSize:"13px",marginBottom:"6px",marginRight:"4%"}}/><input type="text" value={diffCommitNew} onChange={e=>setDiffCommitNew(e.target.value)} placeholder="New commit SHA" style={{width:"48%",padding:"8px 12px",borderRadius:"8px",border:"1px solid #be185d",background:codebg,color:text,fontSize:"13px",marginBottom:"8px"}}/><button onClick={fetchTimeDiff} disabled={diffLoading||!diffRepoUrl} style={{padding:"6px 14px",borderRadius:"8px",border:"1px solid #be185d",background:"transparent",color:"#be185d",cursor:"pointer",fontSize:"12px",fontWeight:"700"}}>{diffLoading?"Comparing...":"Compare Commits"}</button>{diffResult&&<div style={{marginTop:"12px"}}>{diffResult.error?<p style={{color:"#f87171",fontSize:"13px"}}>{diffResult.error}</p>:<div><p style={{color:"#be185d",fontWeight:"700",fontSize:"14px",marginBottom:"8px"}}>{diffResult.diff_summary}</p>{diffResult.diff_lines&&diffResult.diff_lines.length>0&&<pre style={{background:codebg,borderRadius:"8px",padding:"10px",color:text,fontSize:"11px",whiteSpace:"pre-wrap",overflowX:"auto",maxHeight:"300px",overflowY:"auto"}}>{diffResult.diff_lines.join(String.fromCharCode(10))}</pre>}<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{diffResult.diff_disclaimer}</p></div>}</div>}</div>}
{mode==="crosslang"&&<div style={{marginTop:"10px"}}><div style={{display:"flex",gap:"8px",marginBottom:"6px"}}><select value={clFromLang} onChange={e=>setClFromLang(e.target.value)} style={{padding:"6px",borderRadius:"6px",background:codebg,color:text,border:"1px solid #7c3aed"}}><option value="python">Python</option><option value="javascript">JavaScript</option><option value="php">PHP</option></select><span style={{color:text,alignSelf:"center"}}>to</span><select value={clToLang} onChange={e=>setClToLang(e.target.value)} style={{padding:"6px",borderRadius:"6px",background:codebg,color:text,border:"1px solid #7c3aed"}}><option value="javascript">JavaScript</option><option value="python">Python</option><option value="php">PHP</option></select></div><textarea value={clSource} onChange={e=>setClSource(e.target.value)} placeholder="Paste your source code here" rows={6} style={{width:"100%",padding:"8px 12px",borderRadius:"8px",border:"1px solid #7c3aed",background:codebg,color:text,fontSize:"12px",fontFamily:"monospace",marginBottom:"8px"}}/><button onClick={fetchCrossLang} disabled={clLoading||!clSource} style={{padding:"6px 14px",borderRadius:"8px",border:"1px solid #7c3aed",background:"transparent",color:"#7c3aed",cursor:"pointer",fontSize:"12px",fontWeight:"700"}}>{clLoading?"Translating...":"Translate Code"}</button>{clResult&&<div style={{marginTop:"12px"}}>{clResult.error?<p style={{color:"#f87171",fontSize:"13px"}}>{clResult.error}</p>:<div><p style={{color:"#7c3aed",fontWeight:"700",fontSize:"14px",marginBottom:"8px"}}>{clResult.cross_language_summary}</p>{clResult.translated_code&&<pre style={{background:codebg,borderRadius:"8px",padding:"10px",color:text,fontSize:"11px",whiteSpace:"pre-wrap",overflowX:"auto",maxHeight:"300px",overflowY:"auto"}}>{clResult.translated_code}</pre>}<p style={{color:"#f59e0b",fontSize:"11px",fontWeight:"700",marginTop:"8px"}}>{clResult.cross_language_disclaimer}</p></div>}</div>}</div>}
{mode==="ghissues"&&<div style={{marginTop:"10px"}}><input type="text" value={ghIssuesRepoUrl} onChange={e=>setGhIssuesRepoUrl(e.target.value)} placeholder="https://github.com/owner/repo" style={{width:"100%",padding:"8px 12px",borderRadius:"8px",border:"1px solid #dc2626",background:codebg,color:text,fontSize:"13px",marginBottom:"8px"}}/><button onClick={fetchGhIssues} disabled={ghIssuesLoading} style={{padding:"6px 14px",borderRadius:"8px",border:"1px solid #dc2626",background:"transparent",color:"#dc2626",cursor:"pointer",fontSize:"12px",fontWeight:"700"}}>{ghIssuesLoading?"Fetching...":"Fetch Open Issues"}</button>
{ghIssuesResult&&<div style={{marginTop:"12px"}}>{ghIssuesResult.error?<p style={{color:"#f87171",fontSize:"13px"}}>{ghIssuesResult.error}</p>:<div><p style={{color:"#dc2626",fontWeight:"700",fontSize:"13px",marginBottom:"8px"}}>{ghIssuesResult.issues_summary}</p>{ghIssuesResult.issues&&ghIssuesResult.issues.map((iss,ii)=>(<div key={ii} style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"8px"}}><div style={{color:text,fontSize:"13px",fontWeight:"700"}}>#{iss.number}: {iss.title}</div><button onClick={()=>fetchIssueFix(iss)} style={{marginTop:"6px",padding:"4px 10px",borderRadius:"6px",border:"1px solid #dc2626",background:"transparent",color:"#dc2626",cursor:"pointer",fontSize:"11px"}}>Suggest Fix</button>{selectedIssue&&selectedIssue.number===iss.number&&<div style={{marginTop:"8px"}}>{issueFixLoading?<span style={{color:subtext,fontSize:"11px"}}>Loading suggestion...</span>:issueFixResult&&(issueFixResult.error?<p style={{color:"#f87171",fontSize:"11px"}}>{issueFixResult.error}</p>:<div><p style={{color:text,fontSize:"11px",whiteSpace:"pre-wrap"}}>{issueFixResult.suggested_fix}</p><p style={{color:subtext,fontSize:"10px",fontStyle:"italic",marginTop:"6px"}}>{issueFixResult.fix_disclaimer}</p></div>)}</div>}</div>))}</div>}</div>}</div>}
<label htmlFor="fileInput" style={{cursor:"pointer",color:"#38bdf8"}}>
Click to select files (multiple allowed)
</label>
{mode==="regframework"&&<div style={{marginTop:"12px",marginBottom:"12px"}}><label style={{color:subtext,fontSize:"12px",marginRight:"8px"}}>Framework:</label><select value={regFramework} onChange={e=>setRegFramework(e.target.value)} style={{padding:"6px 12px",borderRadius:"6px",background:codebg,color:text,border:"1px solid "+border,fontSize:"13px"}}><option value="SBP">SBP (Pakistan)</option><option value="Basel III">Basel III</option><option value="PCI-DSS">PCI-DSS</option><option value="GDPR">GDPR</option></select></div>}
...
<p style={{color:"#f1f5f9",fontSize:"13px",fontWeight:"700",marginBottom:"10px"}}>Smart Workflow Results</p><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px",marginBottom:"12px"}}><div style={{background:"#0f1525",border:"1px solid #1e2d45",borderRadius:"6px",padding:"10px",textAlign:"center"}}><div style={{fontSize:"20px",fontWeight:"700",fontFamily:"monospace",color:"#ef4444"}}>{smartResults.security?.findings?.length||0}</div><div style={{fontSize:"9px",color:"#64748b",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:"3px"}}>Issues Found</div></div><div style={{background:"#0f1525",border:"1px solid #1e2d45",borderRadius:"6px",padding:"10px",textAlign:"center"}}><div style={{fontSize:"20px",fontWeight:"700",fontFamily:"monospace",color:"#22c55e"}}>{smartResults.migrate?.confidence_score||95}%</div><div style={{fontSize:"9px",color:"#64748b",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:"3px"}}>Confidence</div></div><div style={{background:"#0f1525",border:"1px solid #1e2d45",borderRadius:"6px",padding:"10px",textAlign:"center"}}><div style={{fontSize:"20px",fontWeight:"700",fontFamily:"monospace",color:"#f59e0b"}}>{smartResults.debt?.debt_level||"N/A"}</div><div style={{fontSize:"9px",color:"#64748b",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:"3px"}}>Debt Level</div></div></div><div style={{display:"flex",justifyContent:"center",margin:"12px 0"}}><canvas ref={healthChartRef} width="170" height="170" style={{maxWidth:"170px",maxHeight:"170px"}}></canvas></div>{runHistory.length>0&&<div style={{marginTop:"12px",paddingTop:"10px",borderTop:"1px solid #1e2d45"}}><p style={{color:"#94a3b8",fontSize:"10px",fontWeight:"700",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:"6px"}}>Run History</p>{runHistory.map((h,hi)=>(<div key={hi} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:hi<runHistory.length-1?"1px solid #1e2d45":"none"}}><div><div style={{color:"#f1f5f9",fontSize:"11px"}}>{h.filename}</div><div style={{color:"#64748b",fontSize:"9px"}}>{h.timestamp} - {h.issues} issues</div></div><span style={{fontSize:"9px",fontWeight:"700",padding:"2px 7px",borderRadius:"3px",background:"rgba(59,130,246,0.1)",color:"#60a5fa"}}>{h.confidence}%</span></div>))}</div>}
{smartResults.security&&smartResults.security.findings&&<div style={{marginBottom:"10px"}}><p style={{color:"#f1f5f9",fontSize:"12px",fontWeight:"700",marginBottom:"6px"}}>Security Issues ({smartResults.security.findings.length})</p>{smartResults.security.findings.map((f,fi)=>(<div key={fi} style={{display:"flex",gap:"8px",background:"#0f1525",borderRadius:"6px",padding:"8px 10px",marginBottom:"4px",borderLeft:"3px solid "+(f.severity==="High"?"#ef4444":f.severity==="Medium"?"#f59e0b":"#3b82f6")}}><div style={{flex:1}}><div style={{color:"#f1f5f9",fontSize:"12px",fontWeight:"600"}}>{f.issue}</div><div style={{color:"#64748b",fontSize:"10px",marginTop:"2px"}}>Line {f.lines} - {f.occurrences}x</div></div><span style={{fontSize:"9px",fontWeight:"700",padding:"2px 7px",borderRadius:"3px",height:"fit-content",background:f.severity==="High"?"rgba(239,68,68,0.1)":f.severity==="Medium"?"rgba(245,158,11,0.1)":"rgba(59,130,246,0.1)",color:f.severity==="High"?"#ef4444":f.severity==="Medium"?"#f59e0b":"#3b82f6"}}>{f.severity}</span></div>))}</div>}
...
{result.nodes!==undefined&&mode==="depgraph"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#0891b2",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.graph_summary}</p>
{result.nodes&&result.nodes.length>0&&(()=>{const nodes=result.nodes;const cx=160,cy=160,r=120;const angleStep=(2*Math.PI)/nodes.length;const pos={};nodes.forEach((n,i)=>{const ang=i*angleStep-Math.PI/2;pos[n.id]=[cx+r*Math.cos(ang),cy+r*Math.sin(ang)];});const riskColor={High:"#f87171",Medium:"#f59e0b",Low:"#4ade80"};return(<svg viewBox="0 0 320 320" style={{width:"100%",maxWidth:"360px",display:"block",margin:"0 auto"}}>{result.links&&result.links.map((l,li)=>{const s=pos[l.source],t=pos[l.target];if(!s||!t)return null;return(<line key={li} x1={s[0]} y1={s[1]} x2={t[0]} y2={t[1]} stroke={subtext} strokeWidth="1.5" opacity="0.5"/>);})}{nodes.map((n,i)=>{const p=pos[n.id];const rad=n.type==="entry"?7:5+n.dependents*1.5;return(<g key={i}><circle cx={p[0]} cy={p[1]} r={rad} fill={riskColor[n.risk]||"#94a3b8"} stroke={n.type==="entry"?"#38bdf8":"none"} strokeWidth="2"><title>{n.id} ({n.dependents} dependents, {n.risk} risk)</title></circle><text x={p[0]} y={p[1]-rad-4} fill={text} fontSize="9" textAnchor="middle">{n.id.length>12?n.id.slice(0,12)+"...":n.id}</text></g>);})}</svg>);})()}
<div style={{display:"flex",gap:"12px",fontSize:"11px",color:subtext,marginTop:"8px",justifyContent:"center"}}><span>?? High risk</span><span>?? Medium risk</span><span>?? Low risk</span><span>?? Entry point</span></div>
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.graph_disclaimer}</p>
</div>
)}
{result.dna_dimensions!==undefined&&mode==="dna"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#a855f7",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.dna_summary}</p>
{(()=>{const dims=Object.entries(result.dna_dimensions);const cx=150,cy=150,r=110;const angleStep=(2*Math.PI)/dims.length;const pts=dims.map(([k,v],i)=>{const ang=i*angleStep-Math.PI/2;const dist=(v/100)*r;return[cx+dist*Math.cos(ang),cy+dist*Math.sin(ang)];});const gridPts=dims.map((_,i)=>{const ang=i*angleStep-Math.PI/2;return[cx+r*Math.cos(ang),cy+r*Math.sin(ang)];});const polyStr=pts.map(p=>p[0]+","+p[1]).join(" ");const gridStr=gridPts.map(p=>p[0]+","+p[1]).join(" ");return(<svg viewBox="0 0 300 300" style={{width:"100%",maxWidth:"320px",display:"block",margin:"0 auto"}}><polygon points={gridStr} fill="none" stroke={subtext} strokeWidth="1" opacity="0.3"/><polygon points={polyStr} fill="#a855f7" fillOpacity="0.3" stroke="#a855f7" strokeWidth="2"/>{dims.map(([k,v],i)=>{const ang=i*angleStep-Math.PI/2;const lx=cx+(r+22)*Math.cos(ang);const ly=cy+(r+22)*Math.sin(ang);return(<text key={i} x={lx} y={ly} fill={text} fontSize="11" textAnchor="middle">{k}</text>);})}</svg>);})()}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.dna_disclaimer}</p>
</div>
)}
{result.debt_cost_usd!==undefined&&mode==="debtcost"&&(
<div style={{marginTop:"4px"}}>
<div style={{marginBottom:"12px",padding:"14px",borderRadius:"10px",background:"rgba(5,150,105,0.08)",border:"1px solid #059669"}}><div style={{fontSize:"28px",fontWeight:"800",color:"#059669"}}>${result.debt_cost_usd}</div><div style={{color:text,fontSize:"13px",marginTop:"4px"}}>{result.debt_hours} hours (~{result.debt_days} working days) at ${result.hourly_rate_used}/hr</div></div>
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.debt_cost_disclaimer}</p>
</div>
)}
{result.wrapper_code!==undefined&&mode==="strangler"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#7c2d12",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.strangler_summary}</p>
{result.wrapper_code&&<pre style={{background:codebg,borderRadius:"8px",padding:"12px",color:text,fontSize:"12px",whiteSpace:"pre-wrap",overflowX:"auto"}}>{result.wrapper_code}</pre>}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.strangler_disclaimer}</p>
</div>
)}
...
<div style={{marginTop:"4px"}}>
<p style={{color:"#16a34a",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.roi_summary}</p>
<div style={{display:"flex",flexWrap:"wrap",gap:"10px"}}><div style={{background:codebg,borderRadius:"8px",padding:"12px",flex:"1 1 140px"}}><div style={{fontSize:"18px",fontWeight:"800",color:"#f59e0b"}}>${result.migration_cost_usd}</div><div style={{fontSize:"11px",color:subtext}}>Migration Cost</div></div><div style={{background:codebg,borderRadius:"8px",padding:"12px",flex:"1 1 140px"}}><div style={{fontSize:"18px",fontWeight:"800",color:"#dc2626"}}>${result.rebuild_cost_usd}</div><div style={{fontSize:"11px",color:subtext}}>Rebuild Cost</div></div><div style={{background:codebg,borderRadius:"8px",padding:"12px",flex:"1 1 140px"}}><div style={{fontSize:"18px",fontWeight:"800",color:"#8b5cf6"}}>${result.status_quo_3yr_cost_usd}</div><div style={{fontSize:"11px",color:subtext}}>3yr Status-Quo Cost</div></div><div style={{background:codebg,borderRadius:"8px",padding:"12px",flex:"1 1 140px"}}><div style={{fontSize:"18px",fontWeight:"800",color:result.estimated_savings_3yr_usd>=0?"#4ade80":"#f87171"}}>${result.estimated_savings_3yr_usd}</div><div style={{fontSize:"11px",color:subtext}}>3yr Savings</div></div></div>
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.roi_disclaimer}</p>
</div>
)}
{result.recommended_strategy!==undefined&&mode==="strategy"&&(
<div style={{marginTop:"4px"}}>
<div style={{marginBottom:"12px",padding:"14px",borderRadius:"10px",background:"rgba(220,38,38,0.08)",border:"1px solid #dc2626"}}><div style={{fontSize:"28px",fontWeight:"800",color:"#dc2626"}}>{result.recommended_strategy}</div><p style={{color:text,fontSize:"13px",marginTop:"6px",marginBottom:0}}>{result.strategy_reasoning}</p></div>
{result.decision_inputs&&<div style={{display:"flex",flexWrap:"wrap",gap:"8px"}}><span style={{background:codebg,color:text,fontSize:"12px",padding:"6px 12px",borderRadius:"6px"}}>Complexity: {result.decision_inputs.complexity_score}</span><span style={{background:codebg,color:text,fontSize:"12px",padding:"6px 12px",borderRadius:"6px"}}>Debt: {result.decision_inputs.debt_score}</span><span style={{background:codebg,color:text,fontSize:"12px",padding:"6px 12px",borderRadius:"6px"}}>Est. Hours: {result.decision_inputs.estimated_hours}</span><span style={{background:codebg,color:text,fontSize:"12px",padding:"6px 12px",borderRadius:"6px"}}>Risk: {result.decision_inputs.risk_level}</span></div>}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.recommendation_disclaimer}</p>
</div>
)}
{result.boundaries!==undefined&&mode==="boundaries"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#f59e0b",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.boundary_summary}</p>
{result.boundaries&&result.boundaries.map((bd,bdi)=>(<div key={bdi} style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px",borderLeft:"3px solid #f59e0b"}}><span style={{color:"#f59e0b",fontSize:"13px",fontWeight:"700"}}>{bd.suggested_service}</span><p style={{color:text,fontSize:"12px",margin:"4px 0"}}>Functions: {bd.functions.join(", ")}</p><p style={{color:subtext,fontSize:"11px",margin:0}}>{bd.reasoning}</p></div>))}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.boundary_disclaimer}</p>
</div>
)}
{result.readiness_score!==undefined&&mode==="readiness"&&(
<div style={{marginTop:"4px"}}>
<div style={{marginBottom:"12px",padding:"14px",borderRadius:"10px",background:"rgba(139,92,246,0.08)",border:"1px solid #8b5cf6"}}><div style={{fontSize:"32px",fontWeight:"800",color:result.readiness_score>=65?"#4ade80":result.readiness_score>=40?"#f59e0b":"#f87171"}}>{result.readiness_score}/100</div><div style={{color:"#8b5cf6",fontWeight:"700",fontSize:"14px",marginTop:"4px"}}>{result.readiness_verdict}</div></div>
{result.readiness_reasoning&&result.readiness_reasoning.map((rz,rzi)=>(<div key={rzi} style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px",color:text,fontSize:"12px"}}>{rz}</div>))}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.readiness_disclaimer}</p>
</div>
)}
{result.config_issues!==undefined&&mode==="config"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#0891b2",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.config_summary}</p>
{result.config_issues&&result.config_issues.map((ci,cii)=>(<div key={cii} style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px",borderLeft:"3px solid #0891b2"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}><span style={{color:"#0891b2",fontSize:"12px",fontWeight:"700"}}>{ci.issue}</span><span style={{color:subtext,fontSize:"11px"}}>Line {ci.line}</span></div><p style={{color:text,fontSize:"12px",margin:0}}>{ci.suggestion}</p></div>))}
{result.suggested_env_template&&<div style={{marginTop:"10px",background:codebg,borderRadius:"8px",padding:"10px"}}><p style={{color:subtext,fontSize:"11px",marginBottom:"4px"}}>Suggested .env template:</p><pre style={{margin:0,color:"#4ade80",fontSize:"12px",whiteSpace:"pre-wrap"}}>{result.suggested_env_template}</pre></div>}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.config_disclaimer}</p>
</div>
)}
{result.portability_score!==undefined&&mode==="portability"&&(
<div style={{marginTop:"4px"}}>
<div style={{marginBottom:"12px",padding:"14px",borderRadius:"10px",background:"rgba(124,58,237,0.08)",border:"1px solid #7c3aed"}}><div style={{fontSize:"32px",fontWeight:"800",color:result.portability_score>=80?"#4ade80":result.portability_score>=50?"#f59e0b":"#f87171"}}>{result.portability_score}/100</div><div style={{color:"#7c3aed",fontWeight:"700",fontSize:"14px",marginTop:"4px"}}>{result.portability_level}</div></div>
{result.dependency_issues&&result.dependency_issues.map((di,dii)=>(<div key={dii} style={{background:codebg,borderRadius:"8px",padding:"8px 12px",marginBottom:"5px",color:text,fontSize:"12px"}}>{di}</div>))}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.portability_disclaimer}</p>
</div>
)}
{result.platform_issues!==undefined&&mode==="platform"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#0891b2",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.platform_summary}</p>
{result.platform_issues&&result.platform_issues.map((pi,pii)=>(<div key={pii} style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px",borderLeft:"3px solid #0891b2"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}><span style={{color:"#0891b2",fontSize:"12px",fontWeight:"700"}}>{pi.issue}</span><span style={{color:pi.severity==="High"?"#f87171":"#f59e0b",fontSize:"11px",fontWeight:"700"}}>{pi.severity} &bull; Line {pi.line}</span></div><p style={{color:text,fontSize:"12px",margin:0}}>{pi.note}</p></div>))}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.platform_disclaimer}</p>
</div>
)}
{result.total_suggestions!==undefined&&mode==="refactor"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#059669",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.refactor_summary}</p>
{result.refactoring_suggestions&&result.refactoring_suggestions.map((rf,rfi)=>(<div key={rfi} style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px",borderLeft:"3px solid #059669"}}><span style={{color:"#059669",fontSize:"11px",fontWeight:"700"}}>{rf.issue}</span><p style={{color:text,fontSize:"12px",margin:"4px 0 0 0"}}>{rf.suggestion}</p></div>))}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.refactor_disclaimer}</p>
</div>
)}
{result.total_smells!==undefined&&mode==="smells"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#a855f7",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.smell_summary}</p>
{result.code_smells&&result.code_smells.map((sm,smi)=>(<div key={smi} style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px",borderLeft:"3px solid #a855f7"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}><span style={{color:"#a855f7",fontSize:"12px",fontWeight:"700"}}>{sm.type}</span><span style={{color:subtext,fontSize:"11px"}}>{sm.location}</span></div><p style={{color:text,fontSize:"12px",margin:0}}>{sm.detail}</p></div>))}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.smell_disclaimer}</p>
</div>
)}
{result.quality_score!==undefined&&mode==="quality"&&(
<div style={{marginTop:"4px"}}>
<div style={{marginBottom:"12px",padding:"14px",borderRadius:"10px",background:"rgba(13,148,136,0.08)",border:"1px solid #0d9488"}}><div style={{fontSize:"32px",fontWeight:"800",color:result.quality_score>=85?"#4ade80":result.quality_score>=50?"#f59e0b":"#f87171"}}>{result.quality_score}/100</div><div style={{color:"#0d9488",fontWeight:"700",fontSize:"14px",marginTop:"4px"}}>Code Quality Grade: {result.quality_grade}</div></div>
{result.quality_metrics&&<div style={{display:"flex",flexWrap:"wrap",gap:"8px"}}><span style={{background:codebg,color:text,fontSize:"12px",padding:"6px 12px",borderRadius:"6px"}}>Lines: {result.quality_metrics.lines_of_code}</span><span style={{background:codebg,color:text,fontSize:"12px",padding:"6px 12px",borderRadius:"6px"}}>Complexity: {result.quality_metrics.complexity_level}</span><span style={{background:codebg,color:text,fontSize:"12px",padding:"6px 12px",borderRadius:"6px"}}>Long lines: {result.quality_metrics.long_lines_over_100_chars}</span><span style={{background:codebg,color:text,fontSize:"12px",padding:"6px 12px",borderRadius:"6px"}}>Comments: {result.quality_metrics.comment_ratio_percent}%</span></div>}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.quality_disclaimer}</p>
</div>
)}
{result.sandbox_status!==undefined&&mode==="sandbox"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:result.sandbox_status==="Ran successfully"?"#4ade80":"#ea580c",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.sandbox_status}</p>
{result.sandbox_output&&<div style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px"}}><p style={{color:subtext,fontSize:"11px",margin:"0 0 4px 0"}}>Output:</p><pre style={{margin:0,color:text,fontSize:"12px",whiteSpace:"pre-wrap"}}>{result.sandbox_output}</pre></div>}
{result.sandbox_error&&<div style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px",borderLeft:"3px solid #ea580c"}}><p style={{color:subtext,fontSize:"11px",margin:"0 0 4px 0"}}>Error:</p><pre style={{margin:0,color:"#f87171",fontSize:"12px",whiteSpace:"pre-wrap"}}>{result.sandbox_error}</pre></div>}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.sandbox_disclaimer}</p>
</div>
)}
{result.answer!==undefined&&mode==="codeqa"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#059669",fontWeight:"700",fontSize:"13px",marginBottom:"6px"}}>Q: {result.question}</p>
<div style={{background:codebg,borderRadius:"8px",padding:"14px",border:"1px solid #059669"}}><p style={{margin:0,color:text,fontSize:"13px",whiteSpace:"pre-wrap",lineHeight:"1.6"}}>{result.answer}</p></div>
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.qa_disclaimer}</p>
</div>
)}
{result.framework_checks!==undefined&&mode==="regframework"&&(
<div style={{marginTop:"4px"}}>
<p style={{color:"#dc2626",fontWeight:"700",fontSize:"14px",marginBottom:"10px"}}>{result.framework} &mdash; {result.framework_summary}</p>
{result.framework_checks&&result.framework_checks.map((fc,fci)=>(<div key={fci} style={{background:codebg,borderRadius:"8px",padding:"10px",marginBottom:"6px",borderLeft:"3px solid #dc2626"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}><span style={{color:text,fontSize:"12px",fontWeight:"700"}}>{fc.check}</span><span style={{color:fc.status==="Present"?"#4ade80":"#f59e0b",fontSize:"11px",fontWeight:"700"}}>{fc.status}</span></div><p style={{color:subtext,fontSize:"11px",margin:0}}>{fc.note}</p></div>))}
<p style={{color:subtext,fontSize:"11px",fontStyle:"italic",marginTop:"8px"}}>{result.framework_disclaimer}</p>
</div>
)}
...
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}><h3 style={{color:"#0d9488",margin:0}}>Migration Registry Dashboard</h3><div style={{display:"flex",gap:"10px",alignItems:"center"}}><button onClick={fetchDashboard} style={{background:"transparent",border:"1px solid #0d9488",color:"#0d9488",cursor:"pointer",fontSize:"12px",borderRadius:"6px",padding:"4px 10px"}}>Refresh Now</button><button onClick={()=>setShowDashboard(false)} style={{background:"transparent",border:"none",color:subtext,cursor:"pointer",fontSize:"20px"}}>&times;</button></div></div>
